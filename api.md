...
В начале надо понять бизнес-логику, а не только техническую сторону API

Зная бизнес-требования, спецификацию - можно, используя стандартные сценарии - составить тест-кейсы или чек-листы, разбив приложение на подмодули.
...
- Trusted Boundaries
- Можно предположить, что из формы данные уходят на бэкенд, состоящий из нескольких серверов и базы данных, что бэкенд пишет логи и имеет конфиг — это сразу добавит много тест-кейсов более глубокого уровня, чем «введём слишком длинное значение в поле ввода».
- Например, можно с помощью tcpdump смотреть, точно ли трафик идёт на все сервера бэкенда; можно попробовать отправить SQL-инъекцию или битый JSON (естественно, если вдруг JSON вообще есть в контексте задачи); можно проверить, что будет, если логи займут всё свободное место на диске; можно проверить работу системы при зафайрволенном бэкенде, корректность добавления данных в базу и т.д.
- Это уже было выше (в эндпойнтах): В API запросах в явном виде могут передаваться значения параметров. Это отличный повод **выделить классы эквив-сти, границы входных и выходных значений и проверить их.**
- Можно проверить политики безопастности приложения... иногда бывает что на апи забивают или ввиду нехватки времени не накручивают использование ролей и чаще там всего один пользователь... хотя вот мы получили ключики, для того чтобы выполнять необходимые операции. Разработчики не предусмотрели, как разделить роли юзеров при использовании API.
- Через API такая же процедура авторизации как и через другие интерфейсы, иначе баг.
- Эффективный способ локализации багов в вебе если тестить его через Fiddler или похожие прокси и при нахождении бага анализировать API-беседу в самом фиддлере. Сразу понятно кто виноват - бекенд или клиент. Еще полезно лог фиддлера приложить - чтобы было видно, что посылает сервер в ответ.
- Наличие в запросах (в т.ч. в теле) угловых скобок, одинарных кавычек позволяет проверить на XSS- и SQL- иньекции.

## Тестирование безопасности API (тезисы из доклада https://www.youtube.com/watch?v=46N_zodwzKA):
- 
